...
func TestDecoderDecodeAuthHandoffPasswordFrame(t *testing.T) {
	input := []byte{
	...
	dec := encoding.NewDecoder(NewEncoding(0), bytes.NewReader(input))
...
func TestWorldFrameListEncodingModeAndOffset(t *testing.T) {
	r := NewEncoding(100)
	if r.mode != 1 {
...
func TestDecoderDecodeHeartbeatFrame(t *testing.T) {
	input := []byte{
		199, 205, 171, 241, 128, 63,
	}
	expected := []byte("49277 0")
	enc := encoding.NewDecoder(NewEncoding(0), bytes.NewReader(input))
	result := make([]byte, len(expected))
...
func TestEncoderEncode(t *testing.T) {
	...
	enc := encoding.NewEncoder(NewEncoding(0), &result)
